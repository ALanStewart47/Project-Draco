...
void lcd_init(void) {
  ...
  tft_hard_init();  //硬件初始化
  /*soft reset*/
  tft_write_command(SOFTWARE_RESET);  //软重启
  usleep(100000);
  /*exit sleep*/
  tft_write_command(SLEEP_OFF);  //关闭休眠
  usleep(100000);
  /*pixel format*/
  tft_write_command(PIXEL_FORMAT_SET);  //设置格式
  data = 0x55;
  tft_write_byte(&data, 1);

  lcd_set_direction(DIR_YX_RLUD);  //适用于型号为RX-24B8120的显示屏
...
#if 0 			//需要补充ascii0816的外部函数，添加ASCII0816字库
/*写一个字符*/
