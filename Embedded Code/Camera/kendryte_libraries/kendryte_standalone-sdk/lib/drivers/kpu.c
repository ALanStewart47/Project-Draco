...
static volatile uint8_t dmac_done = 0;
static volatile uint8_t kpu_ai_done = 0;
static volatile uint8_t kpu_all_done = 0;
static volatile uint32_t kpu_ai_count = 0;
...
int kpu_continue_irq(void *_task)
{
    kpu_ai_count++;
#ifdef USE_IRQ 
    kpu_continue(_task);
...
int kpu_run(kpu_task_t *v_task, dmac_channel_number_t dma_ch, const void *src, void *dest, plic_irq_callback_t callback)
{
	...
	kpu_run_dma_output(DMAC_CHANNEL4, task->dst, last_layer->dma_parameter.data.dma_total_byte + 1, kpu_run_all_done, task);

    plic_set_priority(IRQN_AI_INTERRUPT, 1);
    plic_irq_register(IRQN_AI_INTERRUPT, kpu_continue_irq, task);
